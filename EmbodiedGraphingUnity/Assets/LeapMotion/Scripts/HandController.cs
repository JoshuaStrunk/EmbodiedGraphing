...
public class HandController : MonoBehaviour {
  ...
  protected const float MM_TO_M = 0.001f;
	...
	//Changes to grab Palm Position - Kumar 
	public Vector normalizedWristPosition; 
	public Vector normalizedMiddleFingerPosition; 
  ...
  protected void UpdateHandModels(Dictionary<int, HandModel> all_hands,
    ...
    for (int h = 0; h < num_hands; ++h) {
      Hand leap_hand = leap_hands[h];
			...
			//palmPosition = (Vector3)leap_hands[h].PalmPosition.x;
			//Finger middleFinger = leap_hands[h].Finger(0);
			//FingerList fl = leap_hands[h].Fingers;
			//fl.FingerType(Finger.FingerType.TYPE_MIDDLE);
			Frame frame = GetFrame(); 
			InteractionBox interactionBox = frame.InteractionBox;

			FingerList middleFingerList = leap_hands[h].Fingers.FingerType(Finger.FingerType.TYPE_MIDDLE);
			Finger middleFinger = middleFingerList[0]; //since there is only one per hand
			Vector middleFingerPosition = new Vector(middleFinger.StabilizedTipPosition.x, middleFinger.StabilizedTipPosition.y, 0.0f);

			Vector normalizedMiddleFingerPosition = interactionBox.NormalizePoint(middleFingerPosition);
			Vector wristPosition = new Vector(leap_hands[h].WristPosition.x, leap_hands[h].WristPosition.y,0.0f);
			Vector normalizedWristPosition = interactionBox.NormalizePoint(wristPosition);
			...
			//fm.SetLeapHand(leap_hands[h]);

			//Finger middleFinger = new Finger(); 
			//middleFinger = leap_hands[h].Finger(2);
			//Debug.Log("Finger X "+middleFinger.StabilizedTipPosition.x+"Finger Y"+middleFinger.StabilizedTipPosition.y);      
